...
class UserExchangeRecordModel extends Model{
	 ...
	 */
	public function updateIngStatus($ids, $username){
		$where = "id in ($ids) AND (status = 1) AND is_del = 0";
		$param = array(
			'status' => '3',
	 ...
	 * @param unknown $username
	 */
	public function getConversionByIDS($ids){
		$where = "`id` in ($ids) AND `status` in (0,1) AND `is_del` = 0";
		$param = array(
			'status' => '3',
			'update_date' => date("Y-m-d H:i:s", time()),
		);
