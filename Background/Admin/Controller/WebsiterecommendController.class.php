 ...
 * 新闻管理
 */
class WebsiterecommendController extends BaseController
{
    /**
     * 分类列表
    ...
    public function add()
    {
    	...
    	if (IS_POST) {
    		...
    		$param['head_img'] = $imgDir;
    		$param['content'] = I("post.content");
    		$param['author'] = I("post.author");
    		$param['create_time'] = date("Y-m-d H:i:s", time());
    		$model = D("WebsiteRecommend");
    		$result = $model->insertData($param);
    		if($result){
    			$this->success("添加成功", U('Websiterecommend/index'));
    		}else{
    			$this->error("添加失败");
    		}
    	}
    ...
    public function update()
    {
    	...
    	if (!IS_POST) {
    		...
    		$result = D('WebsiteRecommend')->getInfo($param);
    	...
    	if (IS_POST) {
    		...
    		$param['author'] = I("post.author");
    		$result = D("WebsiteRecommend")->updateData($where, $param);
    		if($result){
    			$this->success("更新成功", U('Websiterecommend/index'));
    		}else{
    			$this->error("更新失败");
    		}
    	}
    ...
    public function delete()
    {
    	...
    	if($result){
    		$this->success("删除成功", U('Websiterecommend/index'));
    	}else{
    		$this->error("删除失败");
    	}
    }
