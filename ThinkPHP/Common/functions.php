 ...
 * @return mixed
 */
function SU($name,$value='',$options=null) {
	static $_usercache   =   '';
	if(is_array($options)){
		// 缓存操作的同时初始化
		$type       =   isset($options['type'])?$options['type']:'';
		$_usercache      =   Think\Cacheuser::getInstance($type,$options);
	}elseif(is_array($name)) { // 缓存初始化
		$type       =   isset($name['type'])?$name['type']:'';
		$_usercache      =   Think\Cacheuser::getInstance($type,$name);
		return $_usercache;
	}elseif(empty($_usercache)) { // 自动初始化
		$_usercache      =   Think\Cacheuser::getInstance();
	}

	if(''=== $value){ // 获取缓存
		return $_usercache->get($name);
	}elseif(is_null($value)) { // 删除缓存
		return $_usercache->rm($name);
	}else { // 缓存数据
		if(is_array($options)) {
			$expire     =   isset($options['expire'])?$options['expire']:NULL;
		}else{
		...
		return $_usercache->set($name, $value, $expire);
	}
}
