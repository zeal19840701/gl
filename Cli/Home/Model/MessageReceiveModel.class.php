...
class MessageReceiveModel extends Model {
    ...
    public function batchInsertData($data){
    	$res = M($this->trueTableName, '', $this->connection)->addAll($data);
    	return $res;
    }
     ...
     */
    public function insertMessage($type, $sender, $title, $content, $receiver){
    	...
    	if($messageId){
    		if(1 == $type){
    			...
    			$userInfo = D('User')->getUserInfo(array('field'=>'id,jg_id,mobile', 'where'=>array('id'=>$receiver)));
    			$messageData = array();
    			if($userInfo['jg_id']){
    				//向极光推送通知
    				if($userInfo['jg_id']){
