...
class WxPayApi
{
	...
	/**
	 * 接收post数据
	 * 微信是用$GLOBALS['HTTP_RAW_POST_DATA'];
	 * 这个函数接收post数据的
	 */
	public static function _postData(){
		$receipt = $_REQUEST;
		if($receipt==null){
	 ...
	 * 把对象转成数组
	 */
	public static function _objectToArray($array){
		if(is_object($array)) {
			$array = (array)$array;
		}if(is_array($array)) {
			foreach($array as $key=>$value) {
				$array[$key] = self::_objectToArray($value);
			}
		}
		return $array;
	}
	 ...
	 * 格式化参数格式化成url参数
	 */
	public static function _ToUrlParams($param = array())
	{
		$buff = "";
	 ...
	 */
	public static function _MakeSign($param = array())
	{
		//签名步骤一：按字典序排序参数
		ksort($param);
